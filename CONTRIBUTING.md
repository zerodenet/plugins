...
Use the [application form](https://github.com/zerodenet/plugins/issues/new?template=submit-plugin.yml) or copy [the entry template](templates/plugin-entry.json) into the appropriate host directory. Provide the plugin name, purpose, repository, license, authors/maintainers, publisher key, release-source adapter, UI surface ceiling, and capability ceiling. Provide independently verifiable publisher/key ownership, one stable onboarding release, security maintenance, and actual host/platform evidence.
