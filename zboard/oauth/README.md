...
源码位于共享仓库 `zerodenet/plugins` 的 `zboard/oauth/`，使用仓库根目录的 Git 历史，不在插件子目录初始化嵌套仓库。插件 ID 和已有身份绑定保持不变。源码目录与宿主运行时安装目录（例如 `data/plugins/`）分离。仓库规范见 [贡献指南](../../CONTRIBUTING.md) 和 [宿主边界准则](../../docs/governance.md)。
...
```sh
# 在 zboard/oauth 目录运行；也可通过 ZBOARD_DIR 指定宿主检出目录
python3 scripts/package.py --zboard /path/to/zboard --dev-key --platform linux-amd64
python3 scripts/package.py --zboard /path/to/zboard --dev-key --platform linux-arm64
python3 scripts/package.py --zboard /path/to/zboard --dev-key
# 正式发布使用维护者密钥和发布者标识
