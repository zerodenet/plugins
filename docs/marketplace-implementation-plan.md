...
7. P6: publish the static site, snapshot, six host/channel API files, and schemas automatically to the `zero-plugins` Cloudflare Pages project while retaining GitHub Pages; accept both root-path and repository-path outputs, bind `plugins.zerodenet.org`, and verify refresh, outage fallback, and rollback. Report code, commit, push, deployment, and E2E independently.
