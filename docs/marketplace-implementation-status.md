...
- [ ] P6 scheduled/change-triggered builds, review artifacts, the repository-path GitHub Pages fallback, the root-path Cloudflare Pages deployment, and `plugins.zerodenet.org` are accepted. Cache refresh and rollback acceptance remain.

GitHub Pages is published from `main` and remains the fallback URL. Cloudflare deployment uses the organization-level account and token; the project name defaults to `zero-plugins` and may be overridden by the matching Secret or Variable. `plugins.zerodenet.org` is attached and returns the same snapshot as `zero-plugins.pages.dev`. Both Pages outputs publish the static site, unified snapshot, `/api/plugins.json`, six host/channel JSON files, and schemas together. Both hosts retain a read-only fallback to the generated schema-v2 catalogs without weakening local package verification.
